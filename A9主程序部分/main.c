...
void ReleaseResource (int signo)
{
	 ...
	 * 　　1. 初始化:
	 * 　　在Linux下, 线程的互斥量数据类型是pthread_mutex_t. 在使用前,
	 * 要对它进行初始化:
	 * 　　对于静态分配的互斥量, 可以把它设置为PTHREAD_MUTEX_INITIALIZER,
	 * 或者调用pthread_mutex_init.
	 * 　　对于动态分配的互斥量, 在申请内存(malloc)之后,
	 * 通过pthread_mutex_init进行初始化,
	 ...
	 * 只有在没有线程在该条件变量上等待的时候才能注销这个条件变量，
	 * 否则返回EBUSY
	 * 因为Linux实现的条件变量没有分配什么资源，
	 * 所以注销动作只包括检查是否有等待线程
	 * API定义如下：     
