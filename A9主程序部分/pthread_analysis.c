...
/***********************************************************************
 * *处理接收到的信息,因为接收到的信息是按一定规律进行编码的,
 * 所以进行解码后,激活数据库线程,保存数据,激活内存刷新数据,更新实时环境信息,
 * 判断数据是否越界,若越界则激活设备控制线程进行相应的控制。
 ***********************************************************************/ 
void *pthread_analysis (void *arg)
{
	...
	while (1)
	{
		...
		pthread_cond_wait (&cond_analysis, &mutex_analysis);/* 等待唤醒 被谁唤醒？*/
...
//		printf ("wake pthread_analysis wake up\n");
		while (1)
		{
			...
			if ((node = GetLinknode (linkHead)) == NULL)/* 获取节点  */
			{
			...
			if ('e' == buf.msg_type)
			{
				getEnvPackage (&buf);/* 获取环境信息  */
			}
			else if ('r' == buf.msg_type)
			{
				getGoodsPackage (&buf);/* 获取物品信息  */
			}
		}
	}
	return 0;
