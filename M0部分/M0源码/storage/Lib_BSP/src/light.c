...
uint32_t Light; // 全局变量
uint8_t LightGetFinishFlag; // 全局标志
...
void ScanLight(void)
{
	static uint8_t LightFilterCnt = 0;
//	static uint32_t lightsum = 0;
	...
	LightFilterCnt++;
	if(LightFilterCnt >= 10)    //mian 函数循环10次采集一次
	{
		LightFilterCnt = 0;
		Light = tlight;
		//	Light = lightsum / 10;
//		lightsum = 0;
		LightGetFinishFlag = TRUE;
	}
}
...
void InitLight(void)
{
  ...
  light_setRange(LIGHT_RANGE_4000);
	LightGetFinishFlag = FALSE;   //初始化完成
	Light = 0;
}	
