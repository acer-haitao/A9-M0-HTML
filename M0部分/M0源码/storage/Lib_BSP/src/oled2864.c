...
//  Patterns
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
const unsigned char UniV[][48]={			// UniV OLED
	,
};
...
void Show_Font57(unsigned char a, unsigned char b, unsigned char c, unsigned char d)
{
	...
	switch(a)
	{
		case 1:
			...
			Src_Pointer = (unsigned char *) &Ascii_1[b][0];
			break;
		case 2:
...
//  Initialization
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void OLED_Init_E(void)				// VCC Supplied Externally
{
	/*
	unsigned char i;
...
void OLED_Init_I(void)				// VCC Generated by Internal DC/DC Circuit
{
	...
	*/
	...
	//GPIOSetDir(PORT0, 6, 1);	// SCK0		
	//GPIOSetDir(PORT0, 9, 1);	// MOSI0
	//GPIOSetDir(PORT2, 4, 1);	// CS
	GPIOSetDir(PORT2, 5, 1);	// CD
...
void OLED_Test(void)
{
	const unsigned char Name[]={53,78,73,86,73,83,73,79,78,0};					// Univision
								

	Fill_RAM(0x00);		// Clear Screen

	// Show Pattern - UniV OLED
	Show_Pattern((unsigned char *)&UniV[0][0],0x02,0x05,XLevel+0x28,0x30);
	Delay(5);
	...
	Delay(5);				
	Continuous_Scroll(0x01,0x04,0x07,0x00,0x20,0x01,0x00,0x02);		// Upward - Top Area
	Delay(5);														// Leftward - Bottom Area
					
	Continuous_Scroll(0x00,0x04,0x07,0x00,0x20,0x1F,0x00,0x02);		// Downward - Top Area
	Delay(5);														// Rightward - Bottom Area
	...
	// Show String - Univision
	Show_String(1,(unsigned char *)&Name,0x03,XLevel+0x25);
}

void InitOled(void)
{
	//OLED
	SPI_IOConfig(0);	 		/*ÅäÖÃspi IO*/
  SPI_Init(0, 8, 2);		/*³õÊ¼»¯spi*/
	OLED_Init_I();				/*³õÊ¼»¯Òº¾§ÆÁ*/
	OLED_ClearScreen();		/*Cortex M0 Òº¾§ÆÁÇåÆÁ*/
}
